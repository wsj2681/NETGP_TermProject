...
int main()
{
	...
	while (true)
	{
		...
		cout <<
			"\n[TCP ¼­¹ö] Å¬¶óÀÌ¾ðÆ® Á¢¼Ó: IP ÁÖ¼Ò= " << inet_ntoa(client_addr.sin_addr) <<
...
void GameValueInit()
{
	...
	for (int i = 0; i < MONSTER; i++)//¹ö±× ÁÂÇ¥ ÃÊ±âÈ­
	{
...
void UpdateFunction() 
{
	...
	//¾ÆÀÌÅÛ µå·Ó => ÀÌµ¿ + ¾ÆÀÌÅÛ µå·Ó + ÇÃ·¹ÀÌ¾î Ãæµ¹
	for (int i = 0; i < 20; ++i)
	{
...
>>>>>>> parent of 9429522... ìˆ˜ì—…ì‹œê°„
	...
	for (int i = 0; i < 10; ++i)
	{
		if (item_10_Flag[i] == 0)
		{
			bug[bug_num].x_move = bug_x_move(bug[bug_num], player);// ¹ö±×ÀÇ xÀÌµ¿°ª Á¤ÇÏ±â

		}

		if (item_10_Flag[i] == 1)
		{
			bug[bug_num].x_move = bug_x_move(bug[bug_num], item_10[i]);// ¹ö±×ÀÇ xÀÌµ¿°ª Á¤ÇÏ±â
		}
	}
...
>>>>>>> parent of 9429522... ìˆ˜ì—…ì‹œê°„
	...
	for (int i = 0; i < MONSTER; i++)// ¹ö±× ¿òÁ÷ÀÌ±â
	{
